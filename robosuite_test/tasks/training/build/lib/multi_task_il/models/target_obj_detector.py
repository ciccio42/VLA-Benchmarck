...
class TargetObjDetector(nn.Module):
    ...
    def __init__(
        ...
        # Create target position classification module
        # agent embedding + demo embegging
        target_pos_in_dim = int(2*latent_dim)
        if mlp_cfg.n_layers == 1:
            self._action_module = nn.Sequential(
                nn.Linear(target_pos_in_dim, mlp_cfg.out_dim), nn.ReLU())
        elif mlp_cfg.n_layers == 2:
            self._target_object_classification = nn.Sequential(
                nn.Linear(target_pos_in_dim, mlp_cfg.hidden_dim), nn.ReLU(),
                nn.Linear(mlp_cfg.hidden_dim,
                          mlp_cfg.out_dim), nn.ReLU(),
                nn.Linear(mlp_cfg.out_dim, 4)
            )
        else:
            raise NotImplementedError
        ...
        print('Total params in Classification Module:', params)
    ...
    def forward(
        ...
        # Compute embeddings from agent and context
        ...
        # 2. Perform target object classification
        ...
        target_object_prediction = self._target_object_classification(ac_in)

        out['demo_embed'] = demo_embed
        out['img_embed'] = img_embed
        out['target_obj_pred'] = target_object_prediction
