...
class BYOLModule(nn.Module):
    ...
    def forward(self, embed_out, embed_out_target):
        ...
        feats, feats_aug = [out['img_features']
                            for out in [embed_out, embed_out_target]]
        byol_out['img_byol'] = \
            self.calculate_loss(feats, feats_aug, predict,
                                project, target_project)

        if not self.share_mlp:
            predict, project, target_project = self.post_attn_pred, self.post_attn_proj, self.post_attn_proj_tar
        feats, feats_aug = [out['attn_features']
                            for out in [embed_out, embed_out_target]]
        byol_out['attn_byol'] = \
            self.calculate_loss(feats, feats_aug, predict,
                                project, target_project)

        feats, feats_aug = [out['attn_out_0']
                            for out in [embed_out, embed_out_target]]
        byol_out['intm_byol'] = \
            self.calculate_loss(feats, feats_aug, predict,
                                project, target_project)
        ...
        if self.demo_proj:
            ...
            feats, feats_aug = [out['demo_features'] for out in [
                embed_out, embed_out_target]]  # note this is B, 1, d, H, W
            byol_out['demo_byol'] = \
                self.calculate_task_loss(
                    feats, feats_aug, predict, project, target_project)
        return byol_out
        ...
        if self.p > 0:
            for _ in range(self.p):
                if self.draw_apart:
                    ...
                    shuffled = rearrange(
                        projected_target[idxs], 'B T dim ->(B T) dim')
                    ...
                    tar_demo, tar_obs = projected_target.split(
                        [self._demo_T, obs_T], dim=1)
    ...
    def calculate_task_loss(self, demo, demo_aug, predict, project, target_project):
        demo_mean, demo_mean_aug = [torch.mean(
            dem, dim=1, keepdim=self.share_mlp) for dem in [demo, demo_aug]]
...
class ContrastiveModule(nn.Module):
    ...
    def forward(self, embed_out, embed_out_target):
        sim_out = OrderedDict()
        self.calculate_loss(embed_out, embed_out_target, sim_out,
                            in_key='img_features', out_key='simclr_pre')
        self.calculate_loss(embed_out, embed_out_target, sim_out,
                            in_key='attn_features', out_key='simclr_post')
        self.calculate_loss(embed_out, embed_out_target,
                            sim_out, in_key='attn_out_0', out_key='simclr_intm')
        return sim_out
    ...
    def calculate_loss(self, embed_out, embed_out_target, sim_out=None, in_key='img_features', out_key='pre'):
        ...
        if self.temporal:
            ...
            if self.fix_step > 0:  # eq to ATC
                ...
                obs_idxs = torch.randperm(obs_T)
            img_pos = torch.cat(
                [tar_demo[:, demo_idxs], tar_obs[:, obs_idxs]], dim=1)
