...
class DiscreteMixLogistic(torch.distributions.Distribution):
    ...
    def log_prob(self, value):
        ...
        logit_probs = self._logit_probs.reshape(
            (self._log_scale.shape[0], -1, n_mix))
        means = self._mean.reshape((self._mean.shape[0], -1, n_mix))
        log_scales = torch.clamp(self._log_scale.reshape(
            (self._log_scale.shape[0], -1, n_mix)), min=self._log_scale_min)
        ...
        inner_inner_out = inner_inner_cond * \
            ...
            (1. - inner_inner_cond) * (log_pdf_mid -
                                       np.log((self._num_classes - 1) / 2))
    ...
    def sample(self):
        ...
        temp = logit_probs.data.new(
            logit_probs.size()).uniform_(1e-5, 1.0 - 1e-5)
        ...
        log_scales = self._log_scale.reshape(
            (self._log_scale.shape[0], -1, n_mix))
